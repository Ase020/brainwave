            ...
            {collabContent.map(({ id, title, text }) => (
              <li className="mb-3 py-3" key={id}>
                <div className="flex items-center">
                  ...
                  <h6 className="ml-5 body-2">{title}</h6>
                </div>
          ...
          <div className="relative left-1/2 flex w-[22rem] aspect-square border border-n-6 rounded-full -translate-x-1/2 scale-75 md:scale-100">
            ...
            <ul className="">
              {collabApps.map(({ id, title, icon, width, height }, index) => (
                ...
                >
                  <div
                    className={`relative -top-[1.6rem] flex w-[3.2rem] h-[3.2rem] bg-n-7 border border-n-1/15 rounded-full -rotate-${
                      index * 45
                    }`}
                  >
                    <img
